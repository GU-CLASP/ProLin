...
ruleApplies :: Eq w => Enumerable v => Rule (v+w) -> [(v,Exp (v+w))] -> Context w -> [R w]
ruleApplies (Pi (_v,Zero) dom body) metaTypes ctx =
  ruleApplies (pushLeft <$> body) ((Here,mapLeft There <$> dom):[(There v,mapLeft There <$> t) | (v,t) <- metaTypes]) ctx
    ...
    PSubs i o s ->
      let s' = \case
          ...
          s'' = \case
                   Left x -> s x
                   Right x -> V _
      in ruleApplies (body >>= s') [(There v,t >>= _) | (o -> There v,t) <- metaTypes] ctx -- TODO: remove from the context ()
