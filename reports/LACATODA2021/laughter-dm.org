...
#+begin_abstract
In this paper we argue, how laughter, a ambiguous signal ubiquitous in
...
laughter can be accounted for in dialogue manager as an instances of
short answer, feedbacks and vocalisations accompanying them.
#+end_abstract
* Introduction
# flow sentence
...
components, which brings the uncertainty in user queries that has to
be supported by dialogue manager. For simplicity we will represent the
confidence score $t$ in relation to two confidence threshold levels
($T_1 < T_2$), whereas |RED| would correspond to $t < T_1$, |YELLOW|
...
in dialogue, which express, for instance, explicit signals for
...
or negative, and optional value |content| corresponds to a component
of the common ground in question. For instance, the move |(ICM Per
...
For |RED| confidence score system issues interrogative ICM query, such as "I understood you're asking me about the time, it that correct?". In this case a special type of |QUD| is introduced, namely a question about whether question |q| is correctly understood. 
...
Such type of |QUD| requires a special type of processing to integrate
...
dialogue rules are possible. In case of positive answer to such
query, |UND q| QUD is replaced by |q| and can be further handled
by |produceAnswer| rule. In the negative case, |UND q| is removed
from |QUD| and ICM move about understanding of that the question was
not |q| is issued.
...
for example, in case of language tutoring (see Anki flashcard app,
  ...
  card was hard). We can think of a dialogue system scenario where
