...
* Introduction
Laughter is a very frequent in everyday interactions, for instance, in
Switchboard corpus laughter comes about every 100 words. Laughter is
...
icmINTConfirm: (q : Question) -> (x y : DP) -> 
   ...
   QUD qs ⊸ 
   [  _ :: QUD (Cons (Q Bool x (if x then UND q else UNDN q)) qs);
...
icmINTneg:  (q : Question) -> (x y : DP) ->
   ...
   Agenda as ⊸
   Agenda (Cons (ICM Und Neg (QuestionIsNot q)) as)
#+END_code
