...
(such as "no") always depends on the context.  ("Paris" has does not
...
#+END_code
Above we use Π type binders to declare (meta)variables (written here |(a : Type)
...
the type of the bound variable is specified. (The reader worried about any theoretical difficulty
regarding mixing linear and dependent types is directed to atkey_syntax_2018 and abel_unified_2020.)
