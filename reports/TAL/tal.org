...
management and using linear logic formalisation.
...
proof search in various logical frameworks.  That is, metavariables
...
are available at \url{https://github.com/GU-CLASP/ProLin}.] of the components of our framework:
...
2. a set of rules which function as a core framework for dialogue
   ...
   theoretical account).  The rules which we present below are
   provided to this engine, in the same form (modulo typesetting);
...
citet:lupkowski2017query for a general account---and this ties into the
...
the current set of questions under discussion (|QUD|) if the latest
...
distinguish /questions/ as a general surface form and more contextualised
...
propositions, formalised in an underlying logic (often first-order
...
database of facts. For example, the axiom |(Leave 55 Valand
11.50)| can model the fact that bus 55 leaves from Valand at
11:50. The rule |(Leave x Valand y -> Arrive x CentralStationen
(y+45 minutes))| can represent travelling times on a certain line.
...
/metavariables,/[fn::Here, we use the convention that metavariables start with
a lowercase letter, and constants (including predicates) with an upper case.] which play the role of unknowns for unification:
their value can be fixed to any term for a goal to be reached. For example, the goal |(Leave x Valand y)|
corresponds to a request to list all the buses leaving from
Valand (as |x|) together with their departure time (as |y|).
...
once/. For example, one could have a rule |IsAt x Valand y ⊸ IsAt
x CentralStationen (y+45 minutes)|. Consequently, after firing the above
rule, the premiss |(Is x Valand y)| becomes unavailable for any
...
to the /information state/ of the dialogue participant.[fn::We note, that in linear logic, facts (or hypotheses) do not come in a
hierarchy. Either we have a fact, or we don't. However, in second-order variants of intuitionistic logic, like the one we use, one can
...
even after being used), such as (|IsAt x Valand y ⊸ IsAt
x CentralStationen (y+45 minutes)|) above. Each such rule manipulates a part of the
...
In this instance, a simple "no" answer can be ambiguous, and
...
This is a natural account, because the meaning of short answers
...
#+BEGIN_code
(a : Type) -> (x : a) -> (p : Prop) -> 
  QUD (Q a x p) -> p ⊸ (p ⊗ Answer x (Q a x p))
#+END_code
...
#+BEGIN_code
(a : Type) -> (x : a) -> (p : Prop) -> 
  QUD (Q a x p) -> p -* Answer x (Q a x p)
#+END_code
...
In this section we discuss an alternative kind of responding, which is
...
them. Therefore we use a record syntax to limit their scope, ensuring
that they won't occur in the state update. Such a record can be
understood as a conjunction which additionally binds components to
field names.  Additionally, note the use of the single colon (|:|) for
metavariables and the double colon for information-state hypotheses
(|::|).
...
The above presupposes a clear-cut distinction: if an answer is unique,
it is given; otherwise a clarification request is issued. However,
answers could simply be exhaustive ("Mars or Twix").  If the original
questioners are unhappy with the ambiguity, they are free to issue
more precise questions. In practice, one can easily imagine an
...
question "Where does John live?" with the short answer "Paris". The
questionee may decide that there is some ambiguity about /which/
location one is talking about --- after all there are several places
with this name.  To be able to model this, the |Live| relation needs
to be generalised to be a 3-place predicate, where the country is
specified.
...
ambiguous. One way to lift the ambiguity is to raise the clarification
...
*** Initial state
In general, we start with empty |QUD| and |Agenda|. A non-empty |QUD|
...
taken by his partner |y|.[fn::For now we have a very simple model of turn-taking, which can be
...
#+BEGIN_code
processAssert : (a : Type) -> (x : a) -> (p : Prop) -> (qs : List Question) ->
...
#+BEGIN_code
processShort : (a : Type) -> (x : a) ->  (p : Prop) -> (qs : List Question) -> 
  (dp dp1 : DP) ->  Pending (ShortAnswer a x dp1 dp)   ⊸
  QUD (Cons (Q dp a x p) qs)  ⊸ [  _ :: UserFact p; _ :: QUD qs]
#+END_code
...
Moves    Nil
#+END_code
When the system hears the greeting it can be integrated into
...
#+END_code
Now the system hears the question |(Ask (Q U Time t0 (TT n0 t0
Valand d0)))|. It is domain-specific, and basically requests the
timetable information for the given departure station. Again, we
use |hearAndRemember| rule to itegrate it into state, but also,
because the move is |Ask|, the system sets its QUD to the question
that the move contains with the |pushQUD| rule.
...
specificCR :
  ...
  CR  ⊸ QUD (Cons (Q U Time t  (TT n t s d))  qs)   ⊸
...
Moves  [  Ask (Q S Bus n0 (WantBus n0)) S U
          ...
          CounterGreet S U, Greet U S  ]
#+END_code
The user can reply to this with a short answer |ShortAnswer Bus B55 U
S| or an assertion |Assert (WantBus B55) U S|, which can be integrated
using |processShort| or |processAssert| rule respectively. We show the
state after processing the short answer:
#+BEGIN_code
...
The present work provides a minimal and fine-grained account for
...
uncertainty, which is absent from TTR.  Second, expressing updates
...
/inter alia/ cite:grover94. As it stands, KoS is lacking
...
sometimes awkward to represent. Besides, he does not consider the use
of metavariables to represent uncertainty --- even though Prolog in
...
formalised in a way similar to clarifications. However, in our analysis,
...
relations between questions cite:wisniewski2015semantics, e.g. "Who
killed Bill?" can be responded by "Who was in town?". The cases of
dependencies covered in this study are limited to clarification of
metavariables from the original question. This is meant to serve as a
proof-of-concept rather than thorough coverage of all possible cases
of question dependence. A similar issue concern follow-up questions
that are meant to clarify the type of the metavariable, e.g. "What
does John like? Do you mean foodwise?". Generally, further work is
needed to be carried out in order to extend our system to full-scale
coverage of interrelations between QUDs.
...
engage in grounding interaction, modelled extensively by
citet:larsson2002issue.

Table ref:table:ds originates from citet:ginzburg-nlphandbook, who
...
approach the symbol \checkmark indicates that the current approach
safisfies the benchmark in the corresponding row; $\sim$ that the
benchmark could be met with some caveats, as explained in the text
above for most cases; and --- that the benchmark is not met by a
standard version of the current approach.
...
#+begin_export latex
\acknowledgements{
This research was supported by a grant from the Swedish Research Council for the establishment of the Centre for Linguistic Theory and Studies in Probability (CLASP) at the University of Gothenburg. We also acknowledge support by a public grant overseen by the French National Research Agency (ANR) as part of the program “Investissements d’Avenir” (reference: ANR-10-LABX-0083). It contributes to the IdEx Université de Paris --- ANR-18-IDEX-0001. We also acknowledge a senior fellowship from the Institut Universitaire de France to Ginzburg. 
