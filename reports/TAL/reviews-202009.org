...
TODO (Vlad): ask the editor how much space we can use for these clarifications
...
TODO: cite papers mixing intuitionistic/linear and dependent types.
...
TODO: clarify that. (Importantly there is no backtracking!)

> What are the body and the goal of the rules?


TODO. Perhaps explain currification. (each of the arrow introduces a premiss.)
Also, backtracking is cut after the last premiss matches.
...
Clarify that the implementation is done, but we have small scale applications.
...
TODO: maybe ask Staffan about this?
...
TODO: ok, we can reformulate to clarify that.
...
Ok,  but out of scope. (List as future work if more space?)
...
(We use a standard LISP-like notation)

>  - Similarly, is there a particular reason for using the term
>   "metavariable" instead of "free (non-bound) variable"?

TODO: check. Here metavariable emphasises that it can be subsituted
for anything at a later point. (Free variables may be rigid --- not
substitutable )
...
You can run proof search, but the interaction with external systems is
rudimentary. Add link to the implementation.
