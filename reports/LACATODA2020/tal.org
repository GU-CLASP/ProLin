...
the dialogue manager, which determines which system actions 
are appropriate depending on the current state and the external context.
...
theory/. (TODO: citations?) Our aim is to closely integrate dialogue systems with research in
...
be able to express the rules of cooperative dialog in a concise way,
...
the question unclear, a dialogue with ensue before the question is
