...
type |A| which makes |P x| true. We show several examples in table
...
processShort : (a : Type) -> (x : a) ->  (p : Prop) -> 
  (qs : List Question) -> (dp dp1 : DP) ->
  Pending (ShortAnswer a x dp1 dp)   -o 
  QUD (Cons (Question dp a x p) qs)  -o
  [  _ :: UserFact p;
     _ :: QUD qs];
#+END_code
...
*** Questions and clarifications
Just as we described in ref:sec:, we use uniqueness check to determine
...
One of our main sources of inspiration is Ginzburg's KoS. However we
...
Furthermore, as it stands, KoS is lacking implementations, which, as
we see it, are largely due to the semantic gap between its aims
...
To our knowledge cite:dixon2009plans were the first to advocate the
