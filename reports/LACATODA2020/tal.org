...
# Nowadays people tend to use deep learning systems, but they fall short in the following ways.
# TODO
...
Even though this field is promising, we belive that it is crucial to
use formal tools which are most appropriate for the task: one should
be able to express the rules of various genres of dialog in a concise way,
free, to any possible extent, of irrelevant technical details.
...
view here, and further argue for it in sec:TODO.

We further extend this framework to deal with unclarity and
ambiguity. Indeed, asking a question is typically not done in one
utterance which leaves nothing to interpretation. Rather, an initial
short and possbily ambiguous question is posed. If the answerer deems
the question unclear, a dialogue will ensue before the question is
finally resolved. In technical terms we propose to deal with
question-answering and clarification requests in the context of dialog
management.
...
TODO: add forward references above and outline of the rest below.
...
and the ones that have not been yet publicised. As an
illustration[ref:eq:isu-before-update] is a rough exemplification the
information state of a dialogue system, in Cooper's TTR notation.
#
...
What is expressed here is that the dialogue system was just asked (in the
...
Gameboard (DGB).(TODO: is that true?)] and available to a user as well. What is private is
...
The main benefit of using a rich representation of the information state
...
the case of automatic speech recognition or natural language understanding
...
represents language interaction by a dynamically
changing context. The meaning of an utterance is then how it changes the
...
(DGB). Thus, the information states of the participants
comprise a private part and the dialogue gameboard that represents
information arising from publicized interactions. The DGB tracks, at the
...
Therefore, in our implementation, the information state is represented
by a set we treat the information state as a multiset [?] of /linear
...
point in the scenario, the state can refer to metavariables.
...
value. In this paper we explore the potential of using metavariables
in the context of dialog management, and in particular the
representation of question/answering exchanges.
...
qualifing the predicate in question, for example "maybe", "on tuesdays", etc. (Table ref:tbl:qa-ex, last two
rows).  In this instance |A = Prop -> Prop|. 



# NEGATIVE QUESTIONS
One complication are polar questions phrased in the negative (cite:cooper2012negative):

Doesn't John like Bananas?
...
-> DefiniteYes / no he doesn't DefiniteNo / No -> AmbiguousNo / He
does in the weekend -> Qualifier OnWeekend ):
...
always possible to recieve a biasing answer ("I don't know") or no
...
such as rhethorical and attitudinal questions ("Can you believe who I
met yesterday?"), which are also out of scope.
...
***Ultimately this requires rampant polymorphism (a separate arg for country, county
***district, etc Better to treat this as multiple reference
...
examples). They are added to the initial domain-independent context
outlined above. We also assume that user has the turn at the start. 
#+BEGIN_code
i1  :: TT B55 T15  Gotaplatsen     Johanneberg ;
i2  :: TT B55 T30  CentralStation  SciencePark ;
i3  :: TT B18 T1   Gotaplatsen  Johanneberg;
_   :: QUD Nil;
_   :: Agenda Nil;
_   :: Moves Nil;
_   :: HasTurn USER;
...
_   :: QUD Nil;
_   :: Agenda Nil;
_   :: Moves (Cons (Greet USER SYSTEM));
_   :: HasTurn SYSTEM;
#+END_code
...
i3  :: TT B18 T1   Gotaplatsen  Johanneberg;
_   :: QUD Nil;
_   :: Agenda  (CounterGreet SYSTEM USER);
_   :: Moves   (Cons (Greet USER SYSTEM) Nil);
_   :: HasTurn SYSTEM;
#+END_code

Everything which is on the agenda can be uttered
using |utterAndRemember| rule (via issuing |PushMove m| instruction),
given that the system has the turn. System also hands the turn over to
the user. Therefore, the state becomes:
...
_   :: QUD Nil;
_   :: Agenda  Nil;
_   :: Moves   (Cons  (CounterGreet SYSTEM USER)
                      (Cons (Greet USER SYSTEM) Nil);
_   :: HasTurn USER;
#+END_code
...
the move contains with the |setQUD| rule. 

#+BEGIN_code
i1  :: TT B55 T15  Gotaplatsen     Johanneberg ;
i2  :: TT B55 T30  CentralStation  SciencePark ;
i3  :: TT B18 T1   Gotaplatsen  Johanneberg;
_   :: QUD (Question t (TT n t Gotaplatsen d);
_   :: Agenda  Nil;
_   :: Moves  (Cons  (Ask (Question t (TT n t Gotaplatsen d))
                     (Cons  (CounterGreet SYSTEM USER)
                            (Cons (Greet USER SYSTEM) Nil);
_   :: HasTurn SYSTEM;
#+END_code
...
(This is what TTR tries to do with its "assymetric merge"
  operation).
...
Table~\ref{table:ds} originates from \cite{ginzburg-nlphandbook}, who proposed a series of benchmarks
for comparing different approaches to developing dialogue systems (see section xxx of that paper).\footnote{  The  approaches compared
were FSMs, Frames, Inference-based, and the Information state approach.}
 For each approach the symbol \checkmark 
\ indicates that the approach safisfies the benchmark in the corresponding row; 
$\sim$ that the benchmark could be met with some caveats, as explained in 
the text above; and --- that the benchmark is not met by a standard version 
of the approach. Here we discuss 
...
\end{tabular}
\end{center}\caption{Comparison of dialogue management approaches}
\label{table:ds}
\end{figure} 
